        ...
        <?php
            foreach ($speisen as $spei) {
                ...
                echo "<input type='radio' class='btn-check' name='essen' id='$spei' value='$spei' autocomplete='off' $check>
                    <label class='btn btn-secondary' for='$spei'>$spei</label>";
            }
        ?>
                ...
                <?php
                    foreach ($distanz as $dist) {
                        ...
                        echo "<input type='radio' class='btn-check' name='entfernung' id='$dist' value='$dist' autocomplete='off' $check>
                            <label class='btn btn-secondary' for='$dist'>$dist</label>";
                    }
                ?>
            </div>
                ...
                <?php
                    foreach ($kosten as $kost) {
                        ...
                        echo "<input type='radio' class='btn-check' name='preis' id='$kost' value='$kost' autocomplete='off' $check>
                            <label class='btn btn-secondary' for='$kost'>$kost</label>";
                    }
                ?>
            </div>
        </div>
        ...
        <?php
            foreach ($ernaehrung as $ern) {
                ...
                echo "<input type='radio' class='btn-check' name='veggie' id='$ern' value='$ern' autocomplete='off' $check>
                    <label class='btn btn-secondary' for='$ern'>$ern</label>";
            }
        ?>

        <div class="row">
            <input type="submit" style="margin-top: 1rem; width: 10rem" name="reset" value="Reset">
            <input type="submit" style="margin-top: 1rem; width: 10rem" name="absenden" value="Anzeigen">
        </div>
    </form>
</div>
<?php
...
function absenden() {
    ...
    auswahlkriterien($essen, $entfernung, $preis, $veggie);
    
    ergebnis($essen, $entfernung, $preis, $veggie);
